...
import { actionLabel, conditionLabel } from '../types/routing'
import { useUiMode } from '../contexts/UiModeContext'
import { RuleEditor } from './RuleEditor'
import { PanelSection } from './ui/PanelSection'

const MODE_SUMMARY: Record<string, string> = {
  manual: 'Manual assignment — dispatch via Activity tab',
...
function summarizeConditions(rule: RoutingRule, uiMode: 'standard' | 'expert'): string {
  if (rule.conditions.length === 0) return 'No conditions'
  return rule.conditions
    .map(c => {
      const label = conditionLabel(c.type, uiMode)
...
/** IF/THEN routing rule playbook editor. */
export function RoutingRulesPanel({ ruleSet, ruleHits, operatorSetup, onCommand }: Props) {
    ...
    <div className="p-3 pb-4 space-y-3 overflow-y-auto">
      {mode && (
        <div className="px-3 py-2 text-xs text-text-secondary bg-surface-raised/50 border border-border-default rounded-lg">
          <span className="text-text-primary font-medium">Dispatch mode: </span>
          {MODE_SUMMARY[mode] ?? mode}
          <p className="mt-1 italic">Change dispatch mode in the Auto tab.</p>
        </div>
      )}

      <PanelSection title="Rule playbook">
        <div className="p-3 space-y-3">
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => addRule('dispatch')} className="px-2 py-1 text-xs rounded border border-border-default bg-surface-raised hover:bg-surface-base">
              + Dispatch rule
            </button>
            <button type="button" onClick={() => addRule('reposition')} className="px-2 py-1 text-xs rounded border border-border-default bg-surface-raised hover:bg-surface-base">
              + Reposition rule
            </button>
            <button type="button" onClick={() => onCommand({ type: 'RESET_ROUTING_RULES' })} className="px-2 py-1 text-xs rounded border border-border-default text-text-secondary hover:text-text-primary">
              Reset to defaults
            </button>
          </div>
          ...
          <div className="space-y-2">
            {sorted.map((rule, idx) => (
              <div key={rule.id} className="rounded-lg border border-border-default bg-surface-raised/50">
                {editingId === rule.id ? (
                  <div className="p-2">
                    <RuleEditor rule={rule} onChange={updateRule} onClose={() => setEditingId(null)} />
                  </div>
                ) : (
                  <div className="p-2 space-y-1">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="text-[10px] text-text-secondary">{summarizeConditions(rule, uiMode)}</div>
                        <div className="text-[10px] text-emerald-400/80">→ {actionLabel(rule.action.type, uiMode)}</div>
                      </div>
